...
## AI Team Configuration (autogenerated by team-configurator, 2025-08-02)

**Important: YOU MUST USE subagents when available for the task.**

### Detected Tech Stack
- **Monorepo Architecture**: Turborepo with TypeScript monorepo structure
- **Backend**: Node.js with Express, TypeScript, Prisma ORM, Socket.IO
- **Frontend**: Next.js 14 with React 18, Tailwind CSS, Radix UI components
- **Database**: PostgreSQL with Supabase integration, Redis for caching
- **Real-time**: Socket.IO for game events and chat
- **Testing**: Jest, Playwright E2E, React Testing Library
- **Shared Package**: Design tokens, types, and utilities

### AI Team Assignments

| Task Type | Specialist Agent | Notes |
|-----------|------------------|-------|
| **Backend Development** | `backend-developer` | Node.js/Express API development, Socket.IO real-time features |
| **Frontend Development** | `react-nextjs-expert` | Next.js App Router, Server Components, React development |
| **UI Components & Styling** | `tailwind-frontend-expert` | Tailwind CSS, Radix UI integration, responsive design |
| **Code Quality & Security** | `code-reviewer` | Security audits, code standards, performance checks |
| **Performance Optimization** | `performance-optimizer` | Database queries, Socket.IO optimization, bundle analysis |
| **Project Documentation** | `documentation-specialist` | API docs, architecture guides, setup instructions |
| **Codebase Analysis** | `code-archaeologist` | Legacy code exploration, technical debt assessment |

### Specialized Use Cases

**Game Logic Development**: Use `backend-developer` for werewolf game mechanics, role strategies, and night phase management.

**Real-time Features**: Use `backend-developer` for Socket.IO event handlers, authentication middleware, and chat system enhancements.

**UI/UX Components**: Use `react-nextjs-expert` for React components and `tailwind-frontend-expert` for styling and responsive design.

**Database Operations**: Use `backend-developer` for Prisma schema changes, migrations, and Supabase RLS policy updates.

**Testing & Quality**: Always use `code-reviewer` after significant changes and `performance-optimizer` for scaling concerns.

### Example Commands

Try: `@backend-developer Add player voting system with real-time updates`
Try: `@react-nextjs-expert Create game lobby interface with Next.js App Router`
Try: `@tailwind-frontend-expert Style werewolf role cards with dark mode support`