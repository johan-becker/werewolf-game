-- Fix Function Search Path Issues - FINAL VERSION
-- This ensures all functions have immutable search paths for security

-- Force drop and recreate functions to ensure search_path is properly set

-- 1. Drop all functions first to avoid conflicts
...
DROP FUNCTION IF EXISTS public.set_creator_id();
...
-- Set creator ID trigger function
CREATE FUNCTION public.set_creator_id()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
