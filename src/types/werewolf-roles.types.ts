...
export enum ActionType {
  ...
  HUNTER_SHOOT = 'HUNTER_SHOOT',                  // Jäger schießt beim Tod
...
export interface ActionResult {
  ...
  revealedInfo?: {
    ...
    role?: WerewolfRole;
    team?: Team;
    werewolves?: Array<{              // Für Mädchen-Spionage
      playerId: string;
      username: string;
    }>;
  ...
  effects?: {
    ...
    spyResult?: {                       // Mädchen-Spionage Ergebnis
...
export interface WerewolfSocketEvents {
  ...
  'game:validateConfig': (config: GameRoleConfig, callback: (result: ConfigValidationResult) => void) => void;
  'game:startWithRoles': (callback: (result: any) => void) => void;
  
  // Spieler-Events
  'game:performNightAction': (action: Omit<NightAction, 'id' | 'timestamp' | 'resolved'>, callback: (result: ActionResult) => void) => void;
...
export interface WerewolfServerEvents {
  // Spiel-Status
  'game:rolesAssigned': (assignments: Array<{ playerId: string; role: WerewolfRole; roleInfo: RoleInfo }>) => void;
  'game:phaseChanged': (phase: 'DAY' | 'NIGHT', nightPhase?: NightPhase, dayNumber?: number) => void;
  'game:nightPhaseChanged': (phase: NightPhase, activeRole: WerewolfRole) => void;
  
  // Ereignisse
  'game:playerDied': (playerId: string, cause: 'WEREWOLF' | 'WITCH' | 'VOTE' | 'HUNTER' | 'LOVE') => void;
