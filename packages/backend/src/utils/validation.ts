...
export const validateGameRoles = (roles: any): boolean => {
  ...
  const totalVillagers = (roles.villager || 0) + (roles.seer || 0) + (roles.doctor || 0) + (roles.hunter || 0);
...
export const validateGameTiming = (timing: any): boolean => {
  ...
  return (
    timing.dayPhaseSeconds >= 60 && timing.dayPhaseSeconds <= 600 &&
    timing.nightPhaseSeconds >= 30 && timing.nightPhaseSeconds <= 300 &&
    timing.votingSeconds >= 30 && timing.votingSeconds <= 180
  );
};