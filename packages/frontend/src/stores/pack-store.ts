...
export interface PackActivity {
  ...
  packId: string
  userId: string
  type: 'MEMBER_JOINED' | 'MEMBER_LEFT' | 'TERRITORY_CLAIMED' | 'TERRITORY_LOST' | 'ROLE_CHANGED' | 'TRANSFORMATION' | 'PACK_CREATED'
  message: string
...
export interface PackState {
  ...
  // Member management
  inviteMember: (packId: string, userId: string, role: WerewolfRole, message?: string) => Promise<void>
...
export const usePackStore = create<PackState>()(
  persist(
    immer((set, get) => ({
      ...
      // Member management
      inviteMember: async (packId: string, userId: string, role: WerewolfRole, message?: string) => {
        set((state) => {
          state.isLoading = true
          state.error = null
        })
        ...
        try {
          ...
          set((state) => {
            if (state.userPack) {
              ...
              if (territory) {
                state.userPack.territories = state.userPack.territories.filter(t => t.id !== territoryId)
                state.userPack.statistics.territoryCount--
