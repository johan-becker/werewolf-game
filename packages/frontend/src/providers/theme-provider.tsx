      ...
      if (isTransforming && enableTransformationMode) {
        ...
        if (resolvedTheme === 'dark') {
          ...
          root.style.setProperty(
            '--primary',
            `color-mix(in srgb, var(--blood-500) ${50 + intensity * 50}%, var(--blood-700))`
          );
          root.style.setProperty(
            '--ring',
            `color-mix(in srgb, var(--blood-400) ${30 + intensity * 70}%, var(--blood-600))`
          );
          root.style.setProperty(
            '--background',
            `color-mix(in srgb, var(--shadow-950) ${100 - intensity * 10}%, var(--blood-950))`
          );
        } else {
          // Light mode transformation effects
          root.style.setProperty(
            '--primary',
            `color-mix(in srgb, var(--blood-600) ${60 + intensity * 40}%, var(--blood-800))`
          );
          root.style.setProperty(
            '--ring',
            `color-mix(in srgb, var(--blood-500) ${40 + intensity * 60}%, var(--blood-700))`
          );
          root.style.setProperty(
            '--background',
            `color-mix(in srgb, var(--moonlight-50) ${100 - intensity * 5}%, var(--blood-50))`
          );
        }
      }
    },
