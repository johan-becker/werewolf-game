...
export function TransformationTimer({
  ...
  const getTransformationPhase = useCallback(
    (timeLeft: TimeRemaining): TransformationPhase => {
            ...
            <div>
              ...
              <p className="text-lg font-mono">
                {isTransforming
                  ? 'TRANSFORMING'
                  : size === 'compact'
                    ? (formatTime(timeRemaining) as string)
                    : (() => {
                        const formatted = formatTime(timeRemaining) as {
                          days: number;
                          hours: string;
                          minutes: string;
                          seconds: string;
          ...
          <>
            ...
            {showWarnings && currentPhase !== 'SAFE' && (
              ...
              >
                ...
                />
                <AlertDescription
                  className={cn(currentPhase === 'CRITICAL' ? 'text-blood-800' : 'text-amber-800')}
                >
                  {currentPhase === 'CRITICAL'
